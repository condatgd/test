        ...
        switch (f) {
            ...
            case ".": {
                if(pExpr instanceof ExpressionList) {
                    ...
                    Expression concat = eList.getExpressions().stream().reduce(new StringExpression(""), concatExpressions);
                    return concat;
                }
            }
            case "if": {
            ...
            default:
                ...
                if(fn!=null && fn instanceof FunctionDefinition) {
        ...
        if(head instanceof FunctionCallExpression && pExpr instanceof ExpressionList) {
            ...
            if(headExpressions.size()==argExpressions.size()) {
                for(int i=0; i<headExpressions.size(); i++) {
                    env.put(
                            ((VariableExpression) headExpressions.get(i)).pureValueString(),
